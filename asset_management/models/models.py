...
class Asset(models.Model):
    ...
    category_id = fields.Many2one('asset_management.category', required=True,domain=[('active','=',True)],track_visibility='onchange')
...
#generat mas entries from Generate Asset Entries wizard
    ...
    def generate_mas_entries(self,period_id,post_entries,book_id,run_number_process):
        new_moved_lines=[]
        old_moved_lines=[]
        capitalized_asset=self.env['asset_management.book_assets'].search([('state','=','open'),('book_id','=',book_id),('depreciated_flag','=',True)])
        ...
        for entries in capitalized_asset:
            dep_line = self.env['asset_management.depreciation'].search([('asset_id', '=', entries.asset_id.id),('book_id','=',book_id)
                              ,('depreciation_date', '<=', period_id.end_date),('move_posted_check','=',False)])
            trx_lines=self.env['asset_management.transaction'].search([('asset_id', '=', entries.asset_id.id),('book_id','=',book_id),
    ...
    def post_lines_and_close_asset(self,book_id):
        ...
        for line in self:
            ...
            # asset = line.asset_id
            # book=line.book_id
            book_asset=line.env['asset_management.book_assets'].search([('asset_id','=',line.id),('book_id','=',book_id)])
...
class Book(models.Model):
    ...
    description = fields.Text(required=True,track_visibility='onchange')
    company_id=fields.Many2one('res.company', string='Company',required=True,default=lambda self: self.env['res.company']._company_default_get('asset_management.book'),track_visibility='onchange')
    ...
    calendar_id = fields.Many2one('asset_management.calendar',required=True,on_delete='cascade',track_visibility='onchange')
    calendar_line_id = fields.Many2one('asset_management.calendar_line' ,required=True,on_delete="set_null",string='Period',track_visibility='onchange')
    ...
    gain_analytic_account_id = fields.Many2one('account.analytic.account', string='Analytic Account',track_visibility='onchange')
    gain_analytic_tag_ids = fields.Many2many('account.analytic.tag', string='Analytic tags',track_visibility='onchange')
    loss_analytic_account_id = fields.Many2one('account.analytic.account', string='Analytic Account',track_visibility='onchange')
    loss_analytic_tag_ids = fields.Many2many('account.analytic.tag', string='Analytic tags',track_visibility='onchange')
    ...
    def _compute_fiscal_year(self):
        for record in self:
            if record.calendar_line_id:
                record.fiscal_year =str(datetime.strptime(record.calendar_line_id.start_date, DF).strftime("%Y"))
...
class BookAssets (models.Model):
    ...
    book_id = fields.Many2one('asset_management.book',on_delete= 'cascade',required=True, readonly=True, states={'draft': [('readonly', False)]},track_visibility='always')
    asset_id = fields.Many2one('asset_management.asset',on_delete = 'cascade',readonly=True,string='Asset',track_visibility='always')
    ...
    residual_value = fields.Float(string = "Residual Value",compute='_amount_residual',required=True,track_visibility='onchange')
    salvage_value_amount = fields.Float(compute='_compute_salvage_value_amount',track_visibility='onchange')
    method = fields.Selection(
        [('linear','Linear'),
         ('degressive','Degressive')
         ],required=True,string='Depreciation Method',default='linear',readonly=True, states={'draft': [('readonly', False)]},track_visibility='onchange')
    ...
    salvage_value=fields.Float(string='Salvage Value',readonly=True, states={'draft': [('readonly', False)],'open':[('readonly',False)]},track_visibility='onchange')
    date_in_service = fields.Date(string = 'Date In Service',required=True,readonly=True, states={'draft': [('readonly', False)]},track_visibility='onchange')
    ...
    method_number=fields.Integer(string='Number of Depreciation',help="The number of depreciations needed to depreciate your asset",track_visibility='onchange')
    ...
    method_time = fields.Selection([('number', 'Number of Entries'), ('end', 'Ending Date')], string='Time Method',required=True,default= 'number',track_visibility='onchange',
                                   help="Choose the method to use to compute the dates and number of entries.\n"
                                        "  * Number of Entries: Fix the number of entries and the time between 2 depreciations.\n"
                                        "  * Ending Date: Choose the time between 2 depreciations and the date the depreciations won't go beyond.",readonly=True, states={'draft': [('readonly', False)]})
    state = fields.Selection([('draft', 'Draft'), ('open', 'Capitalize'), ('close', 'Close')], 'Status', required=True,track_visibility='onchange',
...
#constraints on current cost and source line value
    ...
    def create(self, values):
        ...
        if 'assignment_id' not in values:
            raise ValidationError(_('Assignment must be added in book ('+str(record.book_id.name)+')'))
        if 'source_line_ids' not in values:
            raise ValidationError (_('Source line must be added to book ('+str(record.book_id.name)+')'))
        if record.prorate_date < record.book_id.calendar_line_id.start_date :
                                    ...
                                    for ss in old:
                                        if ss.get('id') == assign.id:
                                            values2 ={
                                                ...
                                                'depreciation_expense_analytic_account_id': ss.get('depreciation_expense_analytic_account_id'),
...
class Assignment(models.Model):
    ...
    book_id = fields.Many2one("asset_management.book", string="Book",on_delete = 'cascade',compute="_get_book_name",track_visibility='always')
    asset_id = fields.Many2one("asset_management.asset", string="Asset", on_delete='cascade',compute="_get_asset_name",track_visibility='always')
    depreciation_expense_account=fields.Many2one('account.account',on_delete='set_null',required=True,domain=[('user_type_id','=','Depreciation')],track_visibility='onchange')
    responsible_id = fields.Many2one('hr.employee', on_delete='set_null',track_visibility='onchange')
    location_id = fields.Many2one('asset_management.location',required=True,domain=[('active','=',True)],track_visibility='onchange')
    ...
    depreciation_expense_analytic_account_id = fields.Many2one('account.analytic.account', string='Analytic Account',track_visibility='onchange')
    depreciation_expense_analytic_tag_ids = fields.Many2many('account.analytic.tag', string='Analytic tags',track_visibility='onchange')

    @api.onchange('book_id')
    def _dep_expense_domain(self):
        for record in self:
                return {'domain': {'depreciation_expense_account': [('company_id', '=', record.book_id.company_id.id)],
                                   'depreciation_expense_analytic_account_id': [('company_id', '=', record.book_id.company_id.id)]
    ...
    #     for record in self:
    #         transfer_date=str(datetime.strptime(record.transfer_date,DF).year)
...
#create transaction when changing responsible or location in capitalize asset
    # @api.multi
    # def write(self,values):
    #     # if 'responsible_id' or 'location_id' or 'percentage' in values:
    #     #         self.book_assets_id.assign_change_flag = True
    #     for record in self:
    #         start_date=record.book_assets_id.book_id.calendar_line_id.start_date
    #         if record.book_assets_id.prorate_date < start_date:
    #             if record.date_from:
    #                 if record.date_from < start_date:
    #                     depreciation_line = record.book_assets_id.depreciation_line_ids.sorted(key=lambda l: l.depreciation_date)
    #                     for dep in depreciation_line:
    #                         if dep.depreciation_date >= record.book_assets_id.prorate_date and dep.move_check:
    #                             tag_ids=[]
    #                             for tag in record.depreciation_expense_analytic_tag_ids:
    #                                 tag_ids.append((4, tag.id, 0))
    #                             start_date = datetime.strptime(start_date, DF).date()
    #                             values2 = {
    #                                 'book_assets_id': record.book_assets_id.id,
    #                                 'depreciation_expense_account': record.depreciation_expense_account.id,
    #                                 'responsible_id': record.responsible_id.id,
    #                                 'location_id': record.location_id.id,
    #                                 'comments': record.comments,
    #                                 'depreciation_expense_analytic_account_id': record.depreciation_expense_analytic_account_id.id,
    #                                 'depreciation_expense_analytic_tag_ids': tag_ids,
    #                                 'date_to': start_date + relativedelta(days=-1),
    #                                 'history_flag': True,
    ...
    #                             }
    #                             record.env['asset_management.assignment'].create(values2)
    #                             # record.search([('id','=',record.id)]).unlink
    #
    #                         break
...
class SourceLine(models.Model):
    ...
    book_assets_id = fields.Many2one('asset_management.book_assets', on_delete='cascade')
    asset_id = fields.Many2one('asset_management.asset',on_delete = 'cascade',compute='_get_asset_name',track_visibility='always')
    book_id=fields.Many2one('asset_management.book',on_delete='cascade',compute='_get_book_name',track_visibility='always')
    ...
    invoice_line_ids = fields.Many2one("account.invoice.line", string="Invoice Line",on_delete='cascade',track_visibility='onchange')
...
class Depreciation(models.Model):
    ...
    remaining_value = fields.Float(string='Next Period Depreciation', digits=0, required=True,track_visibility='onchange')
    ...
    dep_run_process_id = fields.Many2one('asset_management.deprunprocess', on_delete='cascade',track_visibility='onchange')
...
# generate entries in account.move
    ...
    def create_move(self, post_move=True):
        ...
        for line in self:
            ...
            category_books=line.env['asset_management.category_books'].search( [('book_id', '=', self.book_id.id), ('category_id', '=',self.book_assets_id.category_id.id)])[0]
            ...
            assignment_in_book = line.env['asset_management.assignment'].search([('book_assets_id','=',line.book_assets_id.id),('date_to','=',False)])
...
class Retirement (models.Model):
    ...
    book_id=fields.Many2one('asset_management.book',on_delete = 'cascade',required=True,domain=[('active','=',True)],track_visibility='always')
    ...
    def _retire_date_check(self):
        if self.retire_date > self.book_id.calendar_line_id.end_date or self.retire_date < self.book_id.calendar_line_id.start_date :
            raise ValidationError(_("Retirement date must be in fiscal period from " + self.book_id.calendar_line_id.start_date+ " to "
...
class CategoryBooks(models.Model):
    ...
    category_id = fields.Many2one('asset_management.category',readonly=True,on_delete='cascade',string='Category',track_visibility='always')
    book_id = fields.Many2one('asset_management.book',on_delete='cascade',string='Book Num',required=True,domain=[('active','=',True)],track_visibility='always')
    asset_cost_account = fields.Many2one('account.account',on_delete='set_null',required=True,domain=[('user_type_id','=','Fixed Assets')],track_visibility='onchange')
    asset_clearing_account = fields.Many2one('account.account', on_delete='set_null',required=True,domain=[('user_type_id','=','Fixed Assets')],track_visibility='onchange')
    depreciation_expense_account = fields.Many2one('account.account', on_delete='set_null',required=True,domain=[('user_type_id','=','Depreciation')],track_visibility='onchange')
    accumulated_depreciation_account = fields.Many2one('account.account', on_delete='set_null',required=True,domain=[('user_type_id','=','Fixed Assets')],track_visibility='onchange')
    ...
    depreciation_method = fields.Selection([('linear','Linear'),('degressive','Degressive')],default='linear',track_visibility='onchange')
    life_months = fields.Integer(required=True,track_visibility='onchange')
    method_time = fields.Selection([('number', 'Number of Entries'), ('end', 'Ending Date')], string='Time Method',required=True,default='number',track_visibility='onchange',
                                   help="Choose the method to use to compute the dates and number of entries.\n"
                                        "  * Number of Entries: Fix the number of entries and the time between 2 depreciations.\n"
    ...
    asset_cost_analytic_account_id = fields.Many2one('account.analytic.account', string='Analytic Account',track_visibility='onchange')
    asset_cost_analytic_tag_ids = fields.Many2many('account.analytic.tag', string='Analytic tags',track_visibility='onchange')
    asset_clearing_analytic_account_id = fields.Many2one('account.analytic.account', string='Analytic Account',track_visibility='onchange')
    asset_clearing_analytic_tag_ids = fields.Many2many('account.analytic.tag', string='Analytic tags',track_visibility='onchange')
    depreciation_expense_analytic_account_id = fields.Many2one('account.analytic.account', string='Analytic Account',track_visibility='onchange')
    depreciation_expense_analytic_tag_ids = fields.Many2many('account.analytic.tag', string='Analytic tags',track_visibility='onchange')
    accumulated_depreciation_analytic_account_id = fields.Many2one('account.analytic.account', string='Analytic Account',track_visibility='onchange')
    accumulated_depreciation_analytic_tag_ids = fields.Many2many('account.analytic.tag', string='Analytic tags',track_visibility='onchange')
    ...
    def onchange_book_id(self):
        if self.book_id:
            ...
            return {'domain':{'asset_cost_account':[('company_id','=',self.book_id.company_id.id),('user_type_id','=','Fixed Assets')],
...
class Transaction (models.Model):
    ...
    category_id = fields.Many2one("asset_management.category", string="Category",on_delete='cascade',track_visibility='always')
...
class RetirementType(models.Model):
    ...
    _inherit = ['mail.thread', 'mail.activity.mixin']
    name=fields.Char(required=True,track_visibility='always')
    proceeds_of_sale_account=fields.Many2one('account.account',on_delete='set_null',required=True,track_visibility='onchange')
    cost_of_removal_account = fields.Many2one('account.account', on_delete='set_null',required=True,track_visibility='onchange')
    proceeds_of_sale_analytic_account_id = fields.Many2one('account.analytic.account', string='Analytic Account',track_visibility='onchange')
    proceeds_of_sale_analytic_tag_ids = fields.Many2many('account.analytic.tag', string='Analytic tags',track_visibility='onchange')
    cost_of_removal_analytic_account_id = fields.Many2one('account.analytic.account', string='Analytic Account',track_visibility='onchange')
    cost_of_removal_analytic_tag_ids = fields.Many2many('account.analytic.tag', string='Analytic tags',track_visibility='onchange')
...
class Calendar(models.Model):
    ...
    def _check_periods(self):
            ...
            for a,b in zip(period,period[1:]):
                a_end_date = datetime.strptime(a.end_date, DF).date() + relativedelta(days=+1)
                if a.end_date > b.start_date :
                    raise ValidationError(_('periods ' + a.name + ' and ' + b.name + ' are not '))
                elif a_end_date != datetime.strptime(b.start_date, DF).date():
...
class CalendarLines(models.Model):
    ...
    def _check_dates(self):
        ...
        if self.end_date < self.start_date:
            raise ValidationError(_("Closing Date cannot be set before Beginning Date in "+self.name))
...
class DepRunProcess(models.Model):
    ...
    process_period_id = fields.Many2one('asset_management.calendar_line',on_delete="cascade",track_visibility='onchange')
